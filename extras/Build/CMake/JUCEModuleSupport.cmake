...
function(_juce_create_pkgconfig_target prefix)
    set(interface_target pkgconfig_${prefix})
    set(namespaced_target juce::${interface_target})
    ...
    add_library(${namespaced_target} ALIAS ${interface_target})
    install(TARGETS ${interface_target} EXPORT JUCE)

    if(TARGET ${tgt})
        set(target_properties INCLUDE_DIRECTORIES LINK_LIBRARIES LINK_OPTIONS COMPILE_OPTIONS)
        foreach(target_property_nominal IN LISTS target_properties)
            set(target_property INTERFACE_${target_property_nominal})
            get_property(was_set TARGET ${tgt} PROPERTY ${target_property} SET)
            if(was_set)
                get_target_property(property_value ${tgt} ${target_property})
                set_target_properties(${interface_target} PROPERTIES ${target_property} "${property_value}")
            endif()
        endforeach()
    endif()
endfunction()
